...
class Beam:
    ...
    def __init__(self, bird: Bird):
        ...
        割愛
        """
        bimg1 = pg.transform.rotozoom(pg.image.load(f"ex03/fig/beam.png"), 0, 2.0)  # 左向き，2倍
        bimg0 = pg.transform.flip(bimg1, True, False)  # 右向き，2倍
        self._imgs = {
            (+1, 0): bimg1,  # 右
            (0, 0): bimg1,  # 右
            (+1, -1): pg.transform.rotozoom(bimg1, 45, 1.0),  # 右上
            (0, -1): pg.transform.rotozoom(bimg1, 90, 1.0),  # 上
            (-1, -1): pg.transform.rotozoom(bimg0, -45, 1.0), # 左上
            (-1, 0): bimg0,  # 左
            (-1, +1): pg.transform.rotozoom(bimg0, 45, 1.0),  # 左下
            (0, +1): pg.transform.rotozoom(bimg1, -90, 1.0),  # 下
            (+1, +1): pg.transform.rotozoom(bimg1, -45, 1.0),  # 右下
        }
        
        self._img = self._imgs[tuple(sum_mvb)]  # 画像surface
        self._rct = self._img.get_rect()  # 画像surfaceに対応したrect
        self._rct.centerx = bird._rct.centerx  
        self._rct.centery = bird._rct.centery
        self._vx, self._vy = sum_mvb[0], sum_mvb[1]
